...
import { TASK_STATUS_LABELS, type TaskDetail, type TaskPriority, type TaskRow, type TaskStatus } from '@daliz/shared';
...
export const PRIORITY_LABELS: Record<TaskPriority, string> = { none: 'No priority', low: 'Low', medium: 'Medium', high: 'High', urgent: 'Urgent' };
export const PRIORITY_RANK: Record<TaskPriority, number> = { urgent: 0, high: 1, medium: 2, low: 3, none: 4 };

export function StatusDot({ status }: { status: TaskStatus }) {
  return <span className={cn('inline-block size-2 shrink-0 rounded-full', STATUS_TONE[status])} aria-hidden />;
}
...
export function PriorityIcon({ priority, className }: { priority: TaskPriority; className?: string }) {
  ...
  const Icon = priority === 'urgent' ? ChevronsUp : priority === 'high' ? ArrowUp : priority === 'medium' ? SignalHigh : ArrowDown;
  const tone = priority === 'urgent' ? 'text-destructive' : priority === 'high' ? 'text-warning' : priority === 'medium' ? 'text-primary' : 'text-muted-foreground';
  return (
    <Tooltip content={`${PRIORITY_LABELS[priority]} priority`}>
      <span className={cn('inline-flex', tone, className)} role="img" aria-label={`${PRIORITY_LABELS[priority]} priority`}>
...
export function AssigneeAvatar({ person, size = 'sm' }: { person: { name: string } | null; size?: 'sm' | 'md' }) {
  if (!person) {
    return (
      <span className={cn('inline-flex items-center justify-center rounded-full border border-dashed text-muted-foreground', size === 'sm' ? 'size-6' : 'size-8')} aria-label="Unassigned" role="img">
...
export function DueLabel({ due, status, today, locale }: { due: string | null; status: TaskStatus; today: string; locale?: string }) {
  if (!due) return null;
  const overdue = due < today && status !== 'done' && status !== 'cancelled';
  const isToday = due === today;
  return (
    <span className={cn('text-xs whitespace-nowrap tabular-nums', overdue ? 'font-medium text-destructive' : isToday ? 'font-medium text-warning' : 'text-muted-foreground')}>
...
export function useTaskCache() {
  ...
  return (task: TaskRow | TaskDetail) => {
    qc.setQueriesData<TaskRow[]>({ queryKey: plannerKeys.tasksAll }, (old) => (Array.isArray(old) ? old.map((t) => (t.id === task.id ? { ...t, ...task } : t)) : old));
    qc.setQueryData<TaskDetail>(plannerKeys.task(task.id), (old) => (old ? { ...old, ...task } : old));
  };
}
...
export function useUpdateTask() {
  ...
  return useMutation({
    mutationFn: ({ task, patch }: { task: Pick<TaskRow, 'id' | 'version'>; patch: Record<string, unknown> }) =>
      api.patch<TaskDetail>(`/planner/tasks/${task.id}`, { ...patch, version: task.version }),
    meta: { silent: true },
    onSuccess: (updated) => {
      write(updated);
      void qc.invalidateQueries({ queryKey: plannerKeys.activity(updated.id) });
      void qc.invalidateQueries({ queryKey: plannerKeys.summary });
      if (updated.parentTaskId) void qc.invalidateQueries({ queryKey: plannerKeys.task(updated.parentTaskId) });
    },
    onError: (e, { task }) => {
...
export function QuickAddTask({
  ...
  const submit = async (e: FormEvent) => {
    ...
    setPending(true);
    try {
      const created = await withIdempotency(idem, body, (key) => api.post<TaskDetail>('/planner/tasks', body, { idempotencyKey: key }));
      setTitle('');
      void qc.invalidateQueries({ queryKey: plannerKeys.tasksAll });
      void qc.invalidateQueries({ queryKey: plannerKeys.project(projectId) });
      if (defaults?.parentTaskId) void qc.invalidateQueries({ queryKey: plannerKeys.task(defaults.parentTaskId) });
      onCreated?.(created);
    } catch (err) {
