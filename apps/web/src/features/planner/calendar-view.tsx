...
export function CalendarView({ project, tasks, onOpen }: { project: ProjectRow; tasks: TaskRow[]; onOpen: (id: string) => void }) {
  ...
  const weekdayNames = Array.from({ length: 7 }, (_, i) => formatIsoDate(days[i]!, { weekday: 'short' }, fmt.locale));
  const undated = tasks.filter((t) => !t.dueDate && t.status !== 'done' && t.status !== 'cancelled').length;
  ...
  return (
    <div className="grid gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {undated ? <span className="ml-auto text-xs text-muted-foreground">{undated} open task{undated === 1 ? '' : 's'} without a due date</span> : null}
      </div>
      <div className="overflow-x-auto">
        <div className="grid min-w-[720px] grid-cols-7 overflow-hidden rounded-xl border" role="grid" aria-label="Tasks by due date">
          {weekdayNames.map((d) => (
          ...
          {days.map((day) => {
            ...
            return (
              <div
                ...
                className={cn('group relative min-h-28 border-r border-b p-1.5 [&:nth-child(7n)]:border-r-0', !inMonth && 'bg-muted/30')}
                ...
                <ul className="grid gap-1">
                  {list.slice(0, 4).map((t) => (
                    <li key={t.id}>
                      ...
                      >
                        <span className={cn('size-1.5 shrink-0 rounded-full', STATUS_TONE[t.status])} aria-hidden />
      ...
      <Dialog open={!!createOn} onOpenChange={(o) => !o && setCreateOn(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Due {createOn ? formatIsoDate(createOn, { dateStyle: 'full' }, fmt.locale) : ''}</DialogDescription>
          </DialogHeader>
          {createOn ? <QuickAddTask projectId={project.id} defaults={{ dueDate: createOn }} autoFocus placeholder="Task title, then Enter" onCreated={() => setCreateOn(null)} /> : null}
        </DialogContent>
      </Dialog>
    </div>
  );
