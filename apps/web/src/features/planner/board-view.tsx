...
export function BoardView({
  ...
  const move = async (
    ...
    qc.setQueryData<TaskRow[]>(listKey, (old) =>
      old ? applyMove(old, task.id, status, afterId, beforeId) : old,
    );
    try {
      const updated = await api.post<TaskDetail>(
        `/planner/tasks/${task.id}/move`,
        { status, afterTaskId: afterId, beforeTaskId: beforeId, version: task.version },
        { silent: false },
      );
      write(updated);
  ...
  const onColumnDrop = (status: TaskStatus) => (e: DragEvent) => {
    ...
    const unchanged =
      task.status === status && current.findIndex((t) => t.id === task.id) === index;
    if (unchanged) return;
        ...
        {STATUS_ORDER.map((status) => {
          ...
          return (
            <section
              ...
              className={cn(
                'flex max-h-[calc(100dvh-16rem)] min-h-48 w-[280px] flex-col rounded-xl border bg-muted/30',
                showDrop && 'border-primary/60 bg-primary/5',
              )}
              onDragOver={onColumnDragOver(status)}
              onDragLeave={(e) => {
                if (!e.currentTarget.contains(e.relatedTarget as Node | null))
                  setDrop((d) => (d?.status === status ? null : d));
              }}
            ...
            >
              <header className="flex items-center gap-2 px-3 pt-3 pb-2">
                ...
                {canCreate ? (
                  <Button
                    ...
                    onClick={() => setAdding(adding === status ? null : status)}
                  >
                    <Plus />
                  </Button>
                ) : null}
              ...
              <ol
                ref={(el) => void (columnRefs.current[status] = el)}
              ...
              >
                ...
                {showDrop && drop.index >= col.filter((x) => x.id !== dragId).length ? (
                  <DropIndicator />
                ) : null}
                {col.length === 0 && !showDrop ? (
...
function BoardCard({
  ...
  return (
    <article
      ...
      className={cn(
        'group relative rounded-lg border bg-card p-3 shadow-xs transition-shadow hover:shadow-md',
    ...
    >
      <button
        type="button"
        onClick={onOpen}
        className="block w-full pr-7 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-sm"
      >
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {task.labels.map((l) => (
              <span
                ...
                style={{ backgroundColor: `${l.color}22`, color: 'var(--foreground)' }}
              >
                <span
