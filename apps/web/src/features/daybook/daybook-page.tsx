...
import { cachedQuery, isNetworkError } from '@/offline/cache';
import { useIsOffline } from '@/offline/connectivity';
import { enqueue, useQueueState } from '@/offline/queue';
...
export function DaybookPage() {
  ...
  const accounts = useQuery({
    ...
    queryFn: () => cachedQuery(keys.accounts, () => api.get<FormAccounts>('/daybook/accounts')),
  });
          ...
          accounts.isError ? (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Income, an expense, or a transfer between your cash and bank accounts.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {accounts.data ? (
                  <EntryForm accounts={accounts.data} />
                ) : (
...
function SummaryCards({
  ...
  summary: DaybookSummary | undefined;
  loading: boolean;
  error: unknown;
  onRetry: () => void;
      ...
      <Card>
        <CardContent className="grid gap-2">
          ...
          {loading || value === undefined ? (
            ...
            <div className={cn('text-2xl font-semibold tracking-tight tabular-nums', tone)}>
              {fmt.money(value)}
            </div>
          )}
          <div className="text-xs text-muted-foreground">
            {formatIsoDate(range.from, { dateStyle: 'medium' }, fmt.locale)} –{' '}
            {formatIsoDate(range.to, { dateStyle: 'medium' }, fmt.locale)} · posted only
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="grid gap-4">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        {stat(
          ...
          summary?.net,
          Wallet,
          summary?.net.startsWith('-') ? 'text-destructive' : undefined,
        )}
      </div>
...
function EntryForm({
  ...
  const waiting =
    queue.pending.filter((o) => o.type === 'daybook.create').length +
    queue.errors.filter((o) => o.type === 'daybook.create').length;
  ...
  const form = useForm<FormValues, unknown, FormOutput>({
    ...
    defaultValues: {
      ...
      moneyAccountId:
        entry?.moneyAccount.id ??
        accounts.money.find((a) => a.subtype === 'bank')?.id ??
        accounts.money[0]?.id ??
        '',
  ...
  const categories =
    kind === 'income'
      ...
      : kind === 'expense'
        ? accounts.expense
        : accounts.money.filter((a) => a.id !== moneyAccountId);
  ...
  const onSubmit = form.handleSubmit(async (v) => {
    ...
    try {
      if (entry) {
        ...
        // Offline, an entry can only be *recorded* as pending, never posted; the operation id
        // is the Idempotency-Key so the replay can't create it twice.
        const queueIt = (key: string) =>
          enqueue(
            'daybook.create',
            { body: { ...body, postImmediately: false } },
            `Daybook: ${body.description} (${body.amount})`,
            key,
          );
        const outcome = await withIdempotency(idem, body, async (key) => {
          if (offline) {
            await queueIt(key);
            return 'queued' as const;
          }
          try {
            return await api.post<DaybookEntryRow>('/daybook/entries', body, {
              idempotencyKey: key,
            });
          } catch (err) {
            if (isNetworkError(err) && !body.postImmediately) {
              await queueIt(key);
              return 'queued' as const;
            }
            if (isNetworkError(err))
              throw new Error(
                'Couldn’t confirm with the server, so this entry isn’t shown as posted. Check the entries list before trying again.',
              );
            throw err;
          }
        });
        if (outcome === 'queued')
          toast('Saved offline as pending', {
            description: 'It will be recorded (not posted) when you’re back online.',
          });
        else
          toast.success(
            outcome.status === 'posted'
              ? `Posted as ${outcome.journalNumber ?? 'a journal entry'}`
              : 'Entry recorded — waiting for approval',
          );
        form.reset({
          ...form.getValues(),
          amount: '',
  ...
  const moneyLabel =
    kind === 'income' ? 'Received into' : kind === 'expense' ? 'Paid from' : 'From account';
  const counterLabel =
    kind === 'income' ? 'Income category' : kind === 'expense' ? 'Expense category' : 'To account';
    ...
    <form onSubmit={onSubmit} className="grid gap-4" noValidate>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <FormField
          label={counterLabel}
          required
          error={e.counterAccountId?.message ? 'Choose one' : undefined}
        >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <FormField
          label={
            kind === 'income' ? 'Received from' : kind === 'expense' ? 'Paid to' : 'Counterparty'
          }
          error={e.counterparty?.message}
        >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        {!entry && offline ? (
          <span className="text-xs text-muted-foreground">
            Offline: the entry is saved on this device and recorded as pending when you reconnect.
          ...
          <span className="text-xs text-muted-foreground">
            {entry
              ...
              : 'An approver will post this entry to the ledger.'}
          </span>
        )}
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={
              !canWrite(entry ? 'daybook.update' : 'daybook.create') || (!!entry && offline)
            }
            title={entry && offline ? 'Needs a connection' : undefined}
          >
...
function EntriesList({ accounts }: { accounts: FormAccounts | undefined }) {
  ...
  const list = useQuery({
    ...
    queryFn: () =>
      cachedQuery(keys.entries(query), () => api.list<DaybookEntryRow>('/daybook/entries', query)),
  ...
  return (
    <Card className="overflow-hidden">
      ...
      {list.isPending ? (
        ...
        <EmptyState
          icon={BookOpen}
          title={filtered ? 'No entries match your filters' : 'No daybook entries yet'}
        />
      ) : (
        <Table aria-busy={list.isFetching || undefined}>
          ...
          <TableBody>
            {list.data.items.map((d) => {
              ...
              const actions = {
                edit: pending && canWrite('daybook.update'),
                void: pending && canWrite('daybook.delete'),
              };
              return (
                <TableRow key={d.id} className={cn(d.status === 'void' && 'opacity-60')}>
                  <TableCell className="whitespace-nowrap">
                    {formatIsoDate(d.entryDate, undefined, fmt.locale)}
                  </TableCell>
                  <TableCell className="max-w-xs">
                    ...
                    <div className="truncate text-xs text-muted-foreground">
                      {[
                        ...
                        d.createdBy ? `by ${d.createdBy.name}` : null,
                      ]
                        .filter(Boolean)
                        .join(' · ')}
                    </div>
                  ...
                  <TableCell className="hidden text-xs text-muted-foreground lg:table-cell">
                    {d.kind === 'income'
                      ? `${d.counterAccount.name} → ${d.moneyAccount.name}`
                      : `${d.moneyAccount.name} → ${d.counterAccount.name}`}
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col items-start gap-1">
                      ...
                      {d.journalEntryId && can('accounting.read') ? (
                        <Link
                          to={`/accounting/entries/${d.journalEntryId}`}
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            aria-label={`Actions for ${d.description}`}
                          >
                            <Ellipsis />
                          </Button>
                        </DropdownMenuTrigger>
      ...
      {list.data && list.data.meta.total > 0 ? (
        <Pagination meta={list.data.meta} onPageChange={setPage} noun="entries" />
      ) : null}
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-3xl">
          ...
          {editing && accounts ? (
            <EntryForm accounts={accounts} entry={editing} onDone={() => setEditing(null)} />
          ) : null}
        </DialogContent>
      </Dialog>
      <ConfirmDialog
...
function KindIcon({ kind }: { kind: DaybookKind }) {
  const Icon =
    kind === 'income' ? ArrowDownLeft : kind === 'expense' ? ArrowUpRight : ArrowLeftRight;
  return (
    <span
      className={cn(
